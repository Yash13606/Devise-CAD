...
export function useOverviewKPIs() {
  return useQuery<KPIMetric[]>({
    ...
    queryFn: async () => {
      await delay(300); // TODO: GET /api/mcp/overview/kpis
      return [
        { label: "Total MCP Calls Today", value: 2847, change: 12.4, trend: "up", trendColor: "green" },
        { label: "Calls Approved", value: 2614, change: 8.2, trend: "up", trendColor: "green" },
        { label: "Calls Blocked", value: 189, change: 23.1, trend: "up", trendColor: "red" },
        { label: "Avg Gateway Latency", value: "8ms", change: -2.1, trend: "down", trendColor: "green" },
      ];
    },
  });
}
...
export function useCallTimeline() {
  return useQuery<TimeSeriesDataPoint[]>({
    ...
    queryFn: async () => {
      await delay(400); // TODO: GET /api/mcp/calls/timeline?range=24h
...
export function useTopServers() {
  return useQuery<ServerCallVolume[]>({
    ...
    queryFn: async () => {
      await delay(300); // TODO: GET /api/mcp/servers/top?limit=5
      return SERVERS.filter(s => s.status === "approved").slice(0, 5).map(s => ({
        serverName: s.name, category: s.category, totalCalls: s.monthlyCallCount,
        approved: Math.floor(s.monthlyCallCount * 0.88), blocked: Math.floor(s.monthlyCallCount * 0.12),
      }));
    },
  });
}
...
export function useMCPServers() {
  return useQuery<MCPServer[]>({
    ...
    queryFn: async () => {
      await delay(350); // TODO: GET /api/mcp/servers
      return SERVERS;
    },
  });
}
...
export function useMCPCalls() {
  return useQuery<MCPCall[]>({
    queryKey: ["mcp", "calls"],
    queryFn: async () => {
      await delay(400); // TODO: GET /api/mcp/calls
      return MOCK_CALLS;
    },
  });
}
...
export function useThreatLog() {
  return useQuery<MCPCall[]>({
    ...
    queryFn: async () => {
      await delay(350); // TODO: GET /api/mcp/threats
      return MOCK_CALLS.filter(c => c.threats.length > 0);
    },
  });
}
...
export function useMCPAlerts() {
  return useQuery<MCPAlert[]>({
    queryKey: ["mcp", "alerts"],
    queryFn: async () => {
      await delay(300); // TODO: GET /api/mcp/alerts
      return MOCK_ALERTS;
    },
  });
}
